...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        item["behavior_text"] = self.data["behavior"][index]

        item["context"], item["context_mask"] = self.preprocess(item["context_text"])

        item["target"] = self.preprocess(item["target_text"], anw=True)
        item["behavior"], item["behavior_label"] = self.preprocess_emo(item["behavior_text"], self.beh_map)
    ...
    def preprocess_emo(self, behavior, beh_map):
        program = [0]*len(beh_map)
        program[beh_map[behavior]] = 1
        return program, beh_map[behavior]
...
def collate_fn(data):
    ...
    ##program
    d["target_program"] = item_info['behavior']
    d["program_label"] = item_info['behavior_label']

    ##text
    d["input_txt"] = item_info['context_text']
    d["target_txt"] = item_info['target_text']
    d["program_txt"] = item_info['behavior_text']
    return d 
...
def prepare_data_seq(batch_size=32):  
    ...
    write_config()
    return data_loader_tra, data_loader_val, data_loader_tst, vocab, len(dataset_train.beh_map)