...
def display_data(opt, lang=None):
    ...
    for i in range(opt['num_examples']):
        ...
        if situation != message[0] and i > 0:
            ...
            sys_behaviors.append(clean_msg(behavior, 'behavior'))
            ...
            behaviors.append(clean_msg(behavior, 'behavior'))
    
        situation = message[0]
        behavior = message[1]
        dialog.append(clean_msg(message[6], 'courteous_dialogues'))
        if lang:
    ...
    return situations, behaviors, sys_behaviors, full_dialogs, dialogs, targets, usr_dialogs, usr_targets, sys_dialogs, sys_targets, sys_situations
...
def transform_data(lang, situations, behaviors, sys_behaviors, dialogs, targets, usr_dialogs, usr_targets, sys_dialogs, sys_targets, sure_situations):
    ...
    behaviors = [beh_map[behavior] for behavior in behaviors]
    sys_behaviors = [beh_map[behavior] for behavior in sys_behaviors]
    ...
    return behaviors, sys_behaviors, dialogs, targets, usr_dialogs, usr_targets, sys_dialogs, sys_targets, dialog_lens, target_lens, usr_dialog_lens, usr_target_lens, sys_dialog_lens, sys_target_lens

def map_to_sentiment(situations, behaviors):
    positives = set([
        'assurance', 'empathy', 'greeting', 'appreciation'])
    negatives = set([
        'apology'])
    ...
    unsure_situations = []
    sentiments = []
    for i, (situation, behavior) in enumerate(zip(situations, behaviors)):
        if behavior in positives:
            sure_situations.append(situation)
            sentiments.append(1)
        elif behavior in negatives:
            sure_situations.append(situation)
            sentiments.append(0)
        else:
...
if __name__ == '__main__':
    ...
    print(opt)
    if not os.path.exists('data/prep/courteous-dialogue/lang.pkl'):
        print("Creating vocab from full text")
        lang = Lang()
    else:
        print("Loading vocab")
        sys.argv = ['']
        lang = load_pkl('data/prep/courteous-dialogue/lang')
        print(len(lang))

    situations, behaviors, sys_behaviors, full_dialogs, dialogs, targets, usr_dialogs, usr_targets, sys_dialogs, sys_targets, sys_situations = display_data(opt, lang)
    ...
    print(split)
    save_path = 'data/prep/courteous-dialogue/{}.{}' # name
    ...
    save_npy(sys_behaviors, save_path.format('sys_behavior_texts', split))
